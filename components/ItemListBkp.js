...
q-list.rounded-borders
    q-item.q-py-md(v-for='item in value', :key='item', @click="selection(item)" clickable, v-ripple)
      q-item-section(avatar, top)
        q-avatar(:icon='icon', :color='color', text-color='white')
      q-item-section
        q-item-label(style="font-weight:400; font-size: 16px" lines='1') {{textfieldDotted(item)}}
        q-item-label.text-grey-6 {{descriptionfieldDotted(item)}}
      q-item-section(side)
        slot(:item="item")
